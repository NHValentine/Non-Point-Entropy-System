...
// Base 12 (Duodecimal) digit generation
...
typedef struct {
    ...
    int min_length;
    int max_length;
    // Boundary metrics for BASE 12
    uint64_t count_at_12;
    uint64_t count_at_144;
    double expected_at_12;
    double expected_at_144;
    double delta_at_12;
    double delta_at_144;
    double delta_pct_at_12;
    double delta_pct_at_144;
} FlushResult;
...
void calculate_stats(FlushResult* result) {
    ...
    // Boundary analysis for BASE 12: check at 12 and 144
    result->count_at_12 = result->length_counts[12];
    result->count_at_144 = result->length_counts[144];
    
    // Expected = average of neighbors
    if (result->count_at_12 > 0 && result->length_counts[11] > 0 && result->length_counts[13] > 0) {
        result->expected_at_12 = (result->length_counts[11] + result->length_counts[13]) / 2.0;
        result->delta_at_12 = result->count_at_12 - result->expected_at_12;
        result->delta_pct_at_12 = (result->delta_at_12 / result->expected_at_12) * 100.0;
    }
    
    if (result->count_at_144 > 0 && result->length_counts[143] > 0 && result->length_counts[145] > 0) {
        result->expected_at_144 = (result->length_counts[143] + result->length_counts[145]) / 2.0;
        result->delta_at_144 = result->count_at_144 - result->expected_at_144;
        result->delta_pct_at_144 = (result->delta_at_144 / result->expected_at_144) * 100.0;
    }
}

int main() {
    printf("NPES_256_XII - Time-Based Sweep Test (Base %d)\n", BASE);
    printf("================================================\n");
    printf("Threads: %d\n", NUM_THREADS);
    ...
    for (int flush_rate = 11; flush_rate <= 111; flush_rate += 2) {
        ...
        printf("Boundary @12: actual=%llu, expected=%.2f, delta=%.2f (%.4f%%)\n",
               (unsigned long long)res->count_at_12, res->expected_at_12, 
               res->delta_at_12, res->delta_pct_at_12);
        printf("Boundary @144: actual=%llu, expected=%.2f, delta=%.2f (%.4f%%)\n",
               (unsigned long long)res->count_at_144, res->expected_at_144,
               res->delta_at_144, res->delta_pct_at_144);
    }
    ...
    fprintf(fp, "{\n");
    fprintf(fp, "  \"test_name\": \"NPES_256_XII\",\n");
    fprintf(fp, "  \"base\": %d,\n", BASE);
    ...
    for (int r = 0; r < result_count; r++) {
        ...
        fprintf(fp, "        \"actual\": %llu,\n", (unsigned long long)res->count_at_12);
        fprintf(fp, "        \"expected\": %.2f,\n", res->expected_at_12);
        fprintf(fp, "        \"delta\": %.2f,\n", res->delta_at_12);
        fprintf(fp, "        \"delta_pct\": %.4f\n", res->delta_pct_at_12);
        fprintf(fp, "      },\n");
        fprintf(fp, "      \"boundary_144\": {\n");
        fprintf(fp, "        \"actual\": %llu,\n", (unsigned long long)res->count_at_144);
        fprintf(fp, "        \"expected\": %.2f,\n", res->expected_at_144);
        fprintf(fp, "        \"delta\": %.2f,\n", res->delta_at_144);
        fprintf(fp, "        \"delta_pct\": %.4f\n", res->delta_pct_at_144);
        fprintf(fp, "      },\n");
    ...
    printf("JSON written to: XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");     //<----- output path
    printf("\nTEST COMPLETE!\n");
