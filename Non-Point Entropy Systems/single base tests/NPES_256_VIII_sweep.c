...
typedef struct {
    ...
    int min_length;
    int max_length;
    // Boundary metrics for BASE 8
    uint64_t count_at_8;
    uint64_t count_at_64;
    double expected_at_8;
    double expected_at_64;
    double delta_at_8;
    double delta_at_64;
    double delta_pct_at_8;
...
void calculate_stats(FlushResult* result) {
    ...
    // Boundary analysis for BASE 8: check at 8 and 64
    result->count_at_8 = result->length_counts[8];
    result->count_at_64 = result->length_counts[64];
    
    // Expected = average of neighbors
    if (result->count_at_8 > 0 && result->length_counts[7] > 0 && result->length_counts[9] > 0) {
        result->expected_at_8 = (result->length_counts[7] + result->length_counts[9]) / 2.0;
        result->delta_at_8 = result->count_at_8 - result->expected_at_8;
        result->delta_pct_at_8 = (result->delta_at_8 / result->expected_at_8) * 100.0;
    }
    
    if (result->count_at_64 > 0 && result->length_counts[63] > 0 && result->length_counts[65] > 0) {
...
int main() {
    printf("NPES_256_VIII - Sweep Test (Base %d)\n", BASE);
    printf("================================================\n");
    printf("Threads: %d\n", NUM_THREADS);
    ...
    for (int flush_rate = 11; flush_rate <= 111; flush_rate += 2) {
        ...
        printf("Boundary @8: actual=%llu, expected=%.2f, delta=%.2f (%.4f%%)\n",
               (unsigned long long)res->count_at_8, res->expected_at_8, 
               res->delta_at_8, res->delta_pct_at_8);
    ...
    fprintf(fp, "{\n");
    fprintf(fp, "  \"test_name\": \"NPES_256_VIII\",\n");
    fprintf(fp, "  \"base\": %d,\n", BASE);
    ...
    for (int r = 0; r < result_count; r++) {
        ...
        fprintf(fp, "      \"boundary_8\": {\n");
        fprintf(fp, "        \"actual\": %llu,\n", (unsigned long long)res->count_at_8);
    ...
    printf("JSON written to: XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");     // <---output path
