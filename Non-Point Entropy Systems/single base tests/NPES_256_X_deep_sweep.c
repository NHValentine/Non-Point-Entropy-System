// NPES_256_Mk-X_fullsweep.c
// Non-Point Entropy System
// Time-based sweep test: 1 second per flush rate (10-120)
...
int main() {
    printf("NPES_256_X_deep_sweep - Time-Based Sweep Test (Base %d)\n", BASE);
    printf("================================================\n");
    printf("Threads: %d\n", NUM_THREADS);
    ...
    for (int flush_rate = 11; flush_rate <= 120; flush_rate += 1) {
        printf("\n=== FLUSH RATE: %d ===\n", flush_rate);
    ...
    // Write JSON
    printf("\n\nWriting JSON output...\n");
    FILE* fp = fopen("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX", "w");     // <----- output path
    if (!fp) {
    ...
    fprintf(fp, "{\n");
    fprintf(fp, "  \"test_name\": \"NPES_256_MK-X_fullsweep\",\n");
    fprintf(fp, "  \"base\": %d,\n", BASE);
    ...
    printf("JSON written to: XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");     // <----- output path
    printf("\nTEST COMPLETE!\n");
